...
transformed parameters {
  ...
  //surviva/;
  vector[S] b_space_phi;//section-specific survival adjustment
  ...
  simplex[S] p_vec_all; //bias corrected proportions of fish by reach.
  //entry
  ...
  p_vec_all = softmax(logit(p_vec) - b_space_phi);

  for(i in 1:M){
    if(i <= ss){
      x_mat_aug[i,1:S] = rep_row_vector(1,S) .* to_row_vector(x_mat[i,1:S]);
    }else{
      x_mat_aug[i,1:S] =  to_row_vector(p_vec_all);
    }
  }

  // Constraints
  p[ : ,1] = inv_logit(logit(p_1) + b1_p * length_aug + x_mat_aug[1:M,1:S] * b_space_phi + eps * sigma);
